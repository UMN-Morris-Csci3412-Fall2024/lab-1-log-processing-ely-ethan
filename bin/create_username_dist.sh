...
if [ ! -s "$temp_file" ]; then
  echo "No usernames found in failed_login_data.txt files." >> debug_output.txt
  rm "$temp_file"
  exit 1
fi
...
./bin/wrap_contents.sh "$temp_file" "username_dist" "$input_dir/username_dist.html"

# Check if username_dist.html was created
if [ ! -f "$input_dir/username_dist.html" ]; then
  echo "Failed to create username_dist.html." >> debug_output.txt
  rm "$temp_file" "$sorted_temp_file" "$uniq_temp_file"
  exit 1
fi
